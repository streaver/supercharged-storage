...
class ExpirableStorage extends BaseStorage {
  ...
  static build(storage, options) {
    if (options.ttl || options.expiration) {
      return new ExpirableStorage(storage, options);
    } else {
      throw new TypeError('Please provide a valid ttl or expiration option');
    }
  }
};

ExpirableStorage.prototype.position = 4;

export default ExpirableStorage;
